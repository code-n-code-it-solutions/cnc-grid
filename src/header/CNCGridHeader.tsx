...
const CNCGridHeader: React.FC<CNCGridHeaderProps> = ({
    ...
    const handleIconClick = (e: React.MouseEvent, index: number) => {
        ...
        const iconElement = iconRefs.current[index];
        if (iconElement) {
            const offsetTop = iconElement.getBoundingClientRect().top;
            const offsetLeft = iconElement.getBoundingClientRect().left;

            // Adjust left to ensure it doesn't go offscreen
            const adjustedLeft = Math.max(offsetLeft - 192, 0);

            setShowMenu({ index, position: { top: offsetTop + 25, left: adjustedLeft } });
        }
    };

    // Additional debugging in the component's render
    useEffect(() => {
        console.log("showMenu state updated:", showMenu);
    }, [showMenu]);

    const handleHideMenu = () => {
        setShowMenu({ index: null, position: null });
            ...
            {colDef.map((col, index) => (
                ...
                >
                    ...
                    {showMenu.index === index && showMenu.position && (
                        <div
                            ref={menuRef}
                            className="absolute bg-white shadow-lg border border-gray-200 rounded-md z-50 p-3 absolute-menu"
                            style={{
                                top: showMenu.position.top,
                                left: showMenu.position.left,
                                boxShadow: '0px 4px 12px rgba(0, 0, 0, 0.1)',
